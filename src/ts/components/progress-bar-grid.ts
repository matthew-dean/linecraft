...
export function progressBar(options: ProgressBarOptions): Component {
  return (ctx: RenderContext) => {
    ...
    const barComponent: Component = (barCtx: RenderContext) => {
    ...
    // Use grid to layout: [1] [flex] [1] [6]
    const gridComponent = grid({ template: [1, '1*', 1, 6], columnGap: 0 }, ...children);
    return gridComponent(ctx);
  };
}
